...
def scenario_parse(config, logger):
    ...
    with open(list_of_scenario_config, 'r') as f:
        ...
        # 添加新的过滤条件：跳过scenario_id=5且route_id=0的情况
        # logger.log('>> Filtering out scenario_id=5 and route_id=0 combinations')
        # data_full = [item for item in data_full if not (item["scenario_id"] == 5 and item["route_id"] == 0)]

    logger.log(f'>> Loading {len(data_full)} data')
    completed_eval_ids = logger.get_completed_eval_ids() if hasattr(logger, 'get_completed_eval_ids') else set(logger.eval_records.keys())
    data_full = [
        item for item in data_full
        if item["data_id"] not in completed_eval_ids and str(item["data_id"]) not in completed_eval_ids
    ]
