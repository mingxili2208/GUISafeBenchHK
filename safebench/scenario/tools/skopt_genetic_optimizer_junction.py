...
class BaseScenarioOptimizer(ABC):
    ...
    def __init__(self, scenario):
        ...
        # CARLA world 引用（用于生成点有效性检查）
        self._world = scenario.ego_vehicle.get_world()
    ...
    def _is_spawn_point_valid(self, location: carla.Location, min_actor_distance: float = 4.0) -> bool:
        """
        检查候选生成点是否有效：
        1. 与场景中已有 actor（车辆/行人）的距离必须 >= min_actor_distance，
           避免生成时与其他 actor 重叠导致 CARLA 拒绝生成。
        2. 生成点必须落在可驾驶车道中心附近，避免贴近路缘/静态障碍。
        """
        try:
            # --- 检查 1：与已有 actor 的最小距离 ---
            ...
            for actor in all_actors:
                ...
                dist = math.sqrt(
                    (actor_loc.x - location.x) ** 2 +
                    (actor_loc.y - location.y) ** 2
                )
            ...
            # 与最近可驾驶车道中心的距离过大，说明该点已经偏到车道外或路缘附近
            projected_distance = waypoint.transform.location.distance(location)
            if projected_distance > 1.5:
                return False

            # 检查横向偏移，避免虽然还能投影到 Driving lane，
            # 但实际上已经贴近车道边缘，导致 try_spawn_actor 失败
            lane_center = waypoint.transform.location
            lane_right = waypoint.transform.get_right_vector()
            delta_x = location.x - lane_center.x
            delta_y = location.y - lane_center.y
            lateral_offset = abs(delta_x * lane_right.x + delta_y * lane_right.y)
            usable_half_lane_width = max(0.5, waypoint.lane_width * 0.5 - 0.8)
            if lateral_offset > usable_half_lane_width:
                return False

        except Exception as e:
            # 防御式处理：获取信息失败时放行，不影响优化流程
            print(f">> [spawn_valid_check] warning: {e}")
            return True
...
class OppositeVehicleOptimizer(BaseScenarioOptimizer):
    ...
    def evaluate_fitness(self, genes: np.ndarray) -> float:
        try:
            ...
            # 生成点有效性检查：过滤掉无法生成 actor 的位置
            if not self._is_spawn_point_valid(actor_start_pos):
                return 1e6
