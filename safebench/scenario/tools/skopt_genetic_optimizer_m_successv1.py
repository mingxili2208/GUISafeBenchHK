...
class SkoptGeneticOptimizer:
    """基于几何计算的遗传算法优化器，maneuveroppositedirection场景的ego追尾第二辆车场景优化 success v1"""
    ...
    def __init__(self, scenario):
        ...
        if self.ego_speed < 1.0:  # 小于1m/s认为是静止或刚起步
            self.ego_speed = 8.0  # 设置为10 m/s (36 km/h) 的合理城市速度
            print(
    ...
    def _get_scenario_specific_bounds(self, scenario):
        ...
        if 'dynamicobjectcrossing' in scenario_name or 'pedestrian' in scenario_name:
            ...
            return [
                (5.0, 20.0),  # [0] 第二辆车初始速度: 18-25 m/s (65-90 km/h)
                (4.0, 8.0),  # [1] 停车触发距离: 20-45米
                (-2.0, 2.0),  # [2] X偏移: ±2米（车道内横向偏移）
