...
class OppositeVehicleRunningRedLight(BasicScenario):
    ...
    def __init__(self, world, ego_vehicle, config, timeout=60):
        super(OppositeVehicleRunningRedLight, self).__init__("OppositeVehicleRunningRedLight-Adv", config, world)
        ...
        self.ego_max_driven_distance = 50
    ...
    def initialize_actorsHK(self):
        ...
        if not self.other_actors:
            print(">> ERROR: Actor spawning failed! The location might be invalid.")
            # 这里可以打印最终的 first_vehicle_transform 的坐标
            print(f"       Failed Transform: {self.actor_transform_list[0].location}")
        if not self.other_actors or len(self.other_actors) == 0:
            print(">> ERROR: Actor spawning failed! Trying fallback position...")

            # 回退方案1：使用原始配置位置（不应用遗传算法偏移）
            ...
            fallback_transform.rotation.yaw = (fallback_transform.rotation.yaw) % 360

            self.actor_transform_list = [fallback_transform]
            self.other_actors = self.scenario_operation.initialize_vehicle_actors(
                self.actor_transform_list, self.actor_type_list
            )
    ...
    def _spawn_test_actor_from_params(self, test_params: Dict):
        """
        根据测试参数生成临时actor
        ...
        参数:
            test_params: {
                ...
                'position_offset': {'x': float, 'y': float, 'yaw': float}
            }

        返回:
            test_actor: carla.Actor 或 None
        """
        try:
            # 复用 initialize_actorsHK 的逻辑来生成测试actor
            # 使用测试参数而不是 self.actions
            ...
            yaw_offset = test_params['position_offset'].get('yaw', 0.0)
            ...
            # 应用角度偏移
            final_yaw = transform.rotation.yaw + yaw_offset
            if final_yaw < 0:
                final_yaw += 360
            if final_yaw > 360:
                final_yaw -= 360
            transform.rotation.yaw = final_yaw

            # 生成临时actor
            walker_bp = self.world.get_blueprint_library().filter('walker.*')[0]
            test_actor = self.world.try_spawn_actor(walker_bp, transform)

            if test_actor is not None:
                # 设置速度
                test_actor.set_target_velocity(carla.Vector3D(x=test_params['actor_speed'], y=0, z=0))

            return test_actor

        except Exception as e:
            print(f">> Failed to spawn test actor: {e}")
            return None
    ...
    def update_behavior(self, scenario_action):
        ...
        if self.trigger_location is not None:
            ...
            distance_to_trigger = ego_location.distance(self.trigger_location)

            # 同时满足两个条件才触发:
            # 1. ego正在移动 (速度 > 0.5 m/s)
            # 2. 距离触发点 <= trigger_distance_threshold
            cur_ego_speed = CarlaDataProvider.get_velocity(self.ego_vehicle)
            if (cur_ego_speed and cur_ego_speed > 0.5 and
                    distance_to_trigger <= self.trigger_distance_threshold):
                self.trigger = True
        if self.trigger:
...
class SignalizedJunctionLeftTurn(BasicScenario):
    ...
    def __init__(self, world, ego_vehicle, config, timeout=60):
        super(SignalizedJunctionLeftTurn, self).__init__("SignalizedJunctionLeftTurn-Adv", config, world)
        self.ego_vehicle = ego_vehicle
        self._map = CarlaDataProvider.get_map()
        self.timeout = timeout
        ...
        self.ego_start_location = ego_vehicle.get_location()
        self.trigger= False
        ...
        # 场景操作
        self.scenario_operation = ScenarioOperation()
        self.ego_max_driven_distance = 120
    ...
    def initialize_actorsHK(self):
        ...
        # 安全回退：如果优化后的位置无法生成 actor，则使用原始配置位置重试
        if not self.other_actors:
            print(">> [SignalizedJunctionLeftTurn] Spawn failed at optimized position, "
                  "retrying with original config transform.")
            original_transform = self.config.other_actors[0].transform
            original_transform.rotation.yaw = original_transform.rotation.yaw % 360
            self.actor_transform_list = [carla.Transform(
                carla.Location(
                    original_transform.location.x,
                    original_transform.location.y,
                    original_transform.location.z
                ),
        ...
        if not self.other_actors:
            raise RuntimeError(
                f"[{self.name}] Failed to spawn adversarial vehicle at both optimized "
                "and original positions. Check map/actor placement."
            )
    ...
    def check_stop_condition(self):
        ...
        if cur_distance >= self._actor_distance:
            return True
        ego_current_location = CarlaDataProvider.get_location(self.ego_vehicle)
        ego_traveled = self.ego_start_location.distance(ego_current_location)

        if ego_traveled >= self.ego_max_driven_distance:
            print(f">> Scenario complete: Ego traveled {ego_traveled:.2f}m (max: {self.ego_max_driven_distance}m)")
            return True
        return False
...
class SignalizedJunctionRightTurn_(BasicScenario):
    ...
    def __init__(self, world, ego_vehicle, config, timeout=60):
        super(SignalizedJunctionRightTurn_, self).__init__("SignalizedJunctionRightTurn-Adv", config, world)
        self.ego_vehicle = ego_vehicle
        self.timeout = timeout
...
class SignalizedJunctionRightTurn(BasicScenario):
    ...
    def __init__(self, world, ego_vehicle, config, timeout=60):
        super(SignalizedJunctionRightTurn, self).__init__("SignalizedJunctionRightTurn-Adv", config, world)
        self.ego_vehicle = ego_vehicle
        self.timeout = timeout
    ...
    def initialize_actorsHK(self):
        ...
        # 安全回退：如果优化后的位置无法生成 actor，则使用原始配置位置重试
        if not self.other_actors:
            print(">> [SignalizedJunctionRightTurn] Spawn failed at optimized position, "
                  "retrying with original config transform.")
            original_transform = self.config.other_actors[0].transform
            original_transform.rotation.yaw = original_transform.rotation.yaw % 360
            self.actor_transform_list = [carla.Transform(
        ...
        if not self.other_actors:
            raise RuntimeError(
                f"[{self.name}] Failed to spawn adversarial vehicle at both optimized "
                "and original positions. Check lane occupancy/static obstacles."
            )
...
class NoSignalJunctionCrossingRoute(BasicScenario):
    """
    无信号路口交叉场景 - 使用遗传算法优化初始状态（包含Yaw角）
    目标：让actor在ego行驶时通过距离触发，制造碰撞
    """

    def __init__(self, world, ego_vehicle, config, timeout=60):
        super(NoSignalJunctionCrossingRoute, self).__init__("NoSignalJunctionCrossingRoute-Adv", config, world)
        self.ego_vehicle = ego_vehicle
        self.timeout = timeout
        ...
        # 触发位置：初始化为config中的位置，优化后会更新为actor的实际位置
        self.trigger_location = None
        if hasattr(config, 'trigger_points') and config.trigger_points:
            self.trigger_location = config.trigger_points[0].location
    ...
    def create_behavior(self, scenario_init_action):
        ...
        if self._should_use_genetic_algorithm(scenario_init_action):
            ...
            # 创建遗传算法优化器（使用NoSignalJunctionOptimizer）
            self.genetic_optimizer = create_optimizer(self)
            ...
            print(f"   - Trigger distance: {self.trigger_distance_threshold:.2f} m")
            print(
                f"   - Position offset: x={self.position_offset['x']:.2f}, y={self.position_offset['y']:.2f}, yaw={self.position_offset['yaw']:.2f}")
    ...
    def initialize_actorsHK(self):
        ...
        # 设置参与者变换和类型
        self.actor_transform_list = [first_vehicle_transform]
        self.actor_type_list = ["vehicle.audi.a2"]
        ...
        self.trigger_location = first_vehicle_transform.location
        print(
            f">> Trigger location set to actor position: ({self.trigger_location.x:.2f}, {self.trigger_location.y:.2f})")
    ...
    def update_behavior(self, scenario_action):
        """
        更新场景行为
        当ego距离actor <= trigger_distance_threshold时触发actor移动
        """
        assert scenario_action is None, f'{self.name} should receive [None] action. A wrong scenario policy is used.'

        # 计算ego到触发点（actor位置）的距离
        if self.trigger_location is not None:
            ego_location = CarlaDataProvider.get_location(self.ego_vehicle)
            distance_to_trigger = ego_location.distance(self.trigger_location)
            ...
            if (cur_ego_speed and cur_ego_speed > 0.5 and
                ...
                if not self.trigger:
                    print(
                        f">> Scenario triggered! Ego distance to actor: {distance_to_trigger:.2f}m (threshold: {self.trigger_distance_threshold:.2f}m)")
                self.trigger = True
