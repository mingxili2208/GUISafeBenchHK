...
class ManeuverOppositeDirection(BasicScenario):
    ...
    def __init__(self, world, ego_vehicle, config, timeout=60):
        ...
        self.stop_distance = 6.0  # 第二辆车停车位置距离ego的距离
    ...
    def create_behavior(self, scenario_init_action):
        ...
        if self._should_use_genetic_algorithm(scenario_init_action):
            ...
            self.trigger_distance_threshold = self.optimized_params['trigger_distance']+6.8
            self.stop_distance=self.optimized_params['trigger_distance']
    ...
    def initialize_actorsHK(self):
        ...
        if hasattr(self, 'position_offset') and self.position_offset:
            ...
            second_actor_transform.rotation.yaw = second_actor_transform.rotation.yaw % 360
        second_actor_transform.rotation.yaw = (second_actor_transform.rotation.yaw) % 360-180   # reverse yaw of the actor
    ...
    def update_behavior(self, scenario_action):
        ...
        # 打印调试信息
        print(
            f">> [update_behavior] Distance: {distance:.2f}m, STOP Trigger threshold: {self.stop_distance:.2f}m, Stop executed: {self._stop_executed}")

        # 使用 if-elif-else 结构确保逻辑互斥
        if self._stop_executed:
            # 第二辆车已经停车，保持停止状态
            self.scenario_operation.go_straight(self.initial_speed, 0)  # 第一辆车继续行驶
            self.scenario_operation.brake(self.other_actors[1])  # 第二辆车保持停车
            print(f">> [update_behavior] State: Vehicle stopped (maintaining brake)")

        elif distance <= self.stop_distance:
            # 触发第二辆车突然停车
            print(f">> [update_behavior] TRIGGERING sudden stop at distance: {distance:.2f}m")
            self.scenario_operation.go_straight(self.initial_speed, 0)  # 第一辆车继续
            # self.scenario_operation.brake(self.other_actors[0])
            self.scenario_operation.brake(self.other_actors[1])  # 第二辆车紧急停车
            self._stop_executed = True

        elif distance < self.trigger_distance_threshold:
            # 在触发区间内，两辆车正常行驶
            print(f">> [update_behavior] State: In trigger zone, both vehicles moving")
            self.scenario_operation.go_straight(self.initial_speed, 0)  # 第一辆车
            self.scenario_operation.go_straight(self.initial_speed, 1)  # 第二辆车

        else:
            # 距离太远，场景未激活，车辆可以保持静止或缓慢行驶
            print(
                f">> [update_behavior] State: Too far ({distance:.2f}m > {self.trigger_distance_threshold:.2f}m), waiting")
            # 选项1: 让车辆保持静止
            # self.scenario_operation.brake(self.other_actors[0])
            # self.scenario_operation.brake(self.other_actors[1])

            # 选项2: 让车辆以初始速度行驶（推荐，避免ego追上时车辆是静止的）
            self.scenario_operation.go_straight(self.initial_speed, 0)
            self.scenario_operation.go_straight(self.initial_speed, 1)
