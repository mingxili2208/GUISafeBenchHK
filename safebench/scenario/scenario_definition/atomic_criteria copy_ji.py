...
class AverageVelocityTest(Criterion):
    ...
    def __init__(self, actor, avg_velocity_success, avg_velocity_acceptable=None, optional=False,
...
class CollisionTest(Criterion):
    ...
    def __init__(self, actor, other_actor=None, other_actor_type=None, optional=False, name="CollisionTest",
                 terminate_on_failure=False):
        """
...
class ActorSpeedAboveThresholdTest(Criterion):
    ...
    def __init__(self, actor, speed_threshold, below_threshold_max_time, name="ActorSpeedAboveThresholdTest",
                 terminate_on_failure=False):
        """
            Class constructor
        """
    ...
    def _set_event_message(event, location):
        ...
        """
        event.set_message('Agent got blocked at (x={}, y={}, z={})'.format(round(location.x, 3), round(location.y, 3),
                                                                           round(location.z, 3)))
...
class OffRoadTest(Criterion):
    ...
    def update(self):
        ...
        park_waypoint = self._map.get_waypoint(current_location, project_to_road=False,
...
class RouteCompletionTest(Criterion):
    ...
    def update(self):
        ...
        # 逐步遍历路线,self._waypoints,直到找到距离当前车辆位置最近的路线点index
            ...
            self._percentage_route_completed = 100.0 * float(self._accum_meters[self._current_index]) / float(
                self._accum_meters[-1])
