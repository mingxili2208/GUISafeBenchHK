...
class Criterion:
    ...
    def stop_sensor(self):
        """
        Optional pre-termination hook for criteria backed by CARLA sensors.
        Default criteria do not own a sensor, so this is a no-op.
        """

    def destroy_sensor(self):
        """
        Optional destroy hook for criteria backed by CARLA sensors.
        Default criteria do not own a sensor, so this is a no-op.
        """
...
class KeepLaneTest(Criterion):
    ...
    def stop_sensor(self):
        if self._lane_sensor is not None:
            try:
                if self._lane_sensor.is_alive:
                    self._lane_sensor.stop()
            except Exception:
                pass
...
class RunningStopTest(Criterion):
    ...
    def is_actor_affected_by_stop(self, actor, stop, multi_step=20):
        ...
        for _ in range(multi_step):
            ...
            next_wps = wpx.next(0.5)
            if not next_wps:
                break  # no further waypoint at this resolution
            next_wp = next_wps[0]
            # stop when approaching an intersection
            if next_wp.is_intersection:
                break
            # advance and append the location to be tested against the trigger volume
            wpx = next_wp
            list_locations.append(wpx.transform.location)
