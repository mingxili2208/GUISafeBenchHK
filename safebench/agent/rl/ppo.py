...
class PPO(BasePolicy):
    ...
    def __init__(self, config, logger):
        ...
        self.state_dim = config['ego_state_dim']  # 状态维度,这里一般为4
        self.action_dim = config['ego_action_dim']  # 动作维度,这里一般为2
        self.clip_epsilon = config['clip_epsilon']  # PPO算法中的裁剪参数
    ...
    def train(self, replay_buffer):
        """
        首先从replay_buffer中采样数据,包括状态(bn_s),动作(bn_a),奖励(bn_r),下一个状态(bn_s_),以及是否终止(bn_s_)
        然后使用旧策略old_policy计算旧动作的概率分布,并估计价值目标value_target和优势advantage
        在更新策略时,计算了新旧策略的概率比率,然后通过裁减这个比率来限制策略的更新幅度,从而避免过大的策略变化
        """
        self.old_policy.load_state_dict(self.policy.state_dict())

        # start to train, use gradient descent without batch size
        for K in range(self.train_iteration):  # 这里意味着replay buffer的数据会被反复利用train_iteration次
            ...
            with torch.no_grad():
                ...
                value_target = bn_r + self.gamma * self.value(bn_s_)  # 目标值的计算
                advantage = value_target - self.value(bn_s)

            # update policy
            mu, std = self.policy(bn_s)  # 新策略的均值和标准差
            n = Normal(mu, std) # 定义正态分布
            log_prob = n.log_prob(bn_a)  # 计算新策略的对数概率
            old_log_prob = old_n.log_prob(bn_a) # 计算旧策略的对数概率
            ratio = torch.exp(log_prob - old_log_prob)  # 重要性比率
            L1 = ratio * advantage  # 重要性比率乘以优势
            ...
            # 更新价值网络的时候,使用了均方误差损失来最小化价值预测和目标价值之间的差异
