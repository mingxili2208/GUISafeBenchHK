...
from safebench.util.torch_util import CUDA, hidden_init
...
class DDPG(BasePolicy):
    ...
    def __init__(self, config, logger):
        ...
        self.state_dim = config['ego_state_dim']  # 状态维度
        self.action_dim = config['ego_action_dim']  # 动作维度
        self.actor_lr = config['actor_lr']  # Actor学习率
        self.critic_lr = config['critic_lr']  # Critic学习率
        self.tau = config['tau']  # 软更新参数
        self.gamma = config['gamma']  # 折扣因子
        self.batch_size = config['batch_size']  # 批量大小
        self.update_iteration = config['update_iteration']  # 更新迭代次数
        self.buffer_start_training = config['buffer_start_training']  # 开始训练的缓冲区大小
        self.epsilon = config['epsilon']  # 探索率

        self.model_id = config['model_id']  # 模型ID
        self.model_path = os.path.join(config['ROOT_DIR'], config['model_path'])  # 模型保存路径
        if not os.path.exists(self.model_path):
            os.makedirs(self.model_path)

        self.actor = CUDA(Actor(self.state_dim, self.action_dim))  # 初始化Actor网络
        self.actor_target = CUDA(Actor(self.state_dim, self.action_dim))  # 初始化目标Actor网络
        self.actor_target.load_state_dict(self.actor.state_dict())  # 同步目标Actor网络参数
        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=self.actor_lr)  # 初始化Actor优化器

        self.critic = CUDA(Critic(self.state_dim, self.action_dim))  # 初始化Critic网络
        self.critic_target = CUDA(Critic(self.state_dim, self.action_dim))  # 初始化目标Critic网络
        self.critic_target.load_state_dict(self.critic.state_dict())  # 同步目标Critic网络参数
        self.critic_optimizer = optim.Adam(self.critic.parameters(), lr=self.critic_lr)  # 初始化Critic优化器
