...
from fnmatch import fnmatch  # 用于文件名匹配
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.fc_mu = nn.Linear(256, action_dim)  # 第二个隐藏层到输出层（均值）的线性变换
        self.fc_std = nn.Linear(256, action_dim)  # 第二个隐藏层到输出层（标准差）的线性变换
        self.relu = nn.ReLU()
        self.tanh = nn.Tanh()
        self.softplus = nn.Softplus()
        self.min_val = 1e-3  # 最小值,用以防止标准差为零
...
class Q(nn.Module):
    # Q网络的输入是状态和动作,输出是Q值,用以评价估计状态-动作对的价值
    def __init__(self, state_dim, action_dim):
...
class SAC(BasePolicy):
    ...
    def __init__(self, config, logger):
        ...
        self.buffer_start_training = config['buffer_start_training']  # 开始训练所需要的缓冲区大小
        self.lr = config['lr']  # 学习率
        self.continue_episode = 0  # 继续训练的episode
        self.state_dim = config['ego_state_dim']  # 状态维度
        self.action_dim = config['ego_action_dim']  # 动作维度
        self.min_Val = torch.tensor(config['min_Val']).float()  # 最小值，用以防止标准差为零
        ...
        self.value_net = CUDA(Critic(self.state_dim))  # 价值网络
        self.Q_net = CUDA(Q(self.state_dim, self.action_dim))
        self.Target_value_net = CUDA(Critic(self.state_dim))  # 目标价值网络

        # 创建优化器
        self.policy_optimizer = optim.Adam(self.policy_net.parameters(), lr=self.lr)  # 策略网络优化器
        self.value_optimizer = optim.Adam(self.value_net.parameters(), lr=self.lr)  # 价值网络优化器
        self.Q_optimizer = optim.Adam(self.Q_net.parameters(), lr=self.lr)  # Q网络优化器

        # 定义损失函数
        self.value_criterion = nn.MSELoss()  # 价值网络损失函数
        self.Q_criterion = nn.MSELoss()  # Q网络损失函数
