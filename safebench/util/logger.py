...
class Logger:
    ...
    def __init__(self, output_dir=None, output_fname='progress.txt', exp_name=None, scenario_category='planning'):
        ...
        output_exists = osp.exists(self.output_dir)
        if not output_exists:
            os.makedirs(self.output_dir)

        self.output_file = open(osp.join(self.output_dir, output_fname), 'a')
        self.runtime_log_file = open(osp.join(self.output_dir, 'runtime.log'), 'a', encoding='utf-8')
        atexit.register(self.output_file.close)
        atexit.register(self.runtime_log_file.close)
        self.log('>> ' + '-' * 40)
        if output_exists:
        ...
        self.log(">> Runtime log to %s" % self.runtime_log_file.name, 'green')
    ...
    def create_eval_dir(self, load_existing_results=True):
        ...
        self.batch_log_file = os.path.join(result_dir, 'batch_results.jsonl')
    ...
    def _normalize_eval_record_keys(self):
        normalized_records = {}
        for key, value in self.eval_records.items():
            try:
    ...
    def get_completed_eval_ids(self):
        completed_ids = set()
        for key in self.eval_records.keys():
            completed_ids.add(key)
            completed_ids.add(str(key))
            try:
                completed_ids.add(int(key))
            except (TypeError, ValueError):
                pass
    ...
    def log(self, msg, color='green'):
        ...
        if self.runtime_log_file is not None:
            ...
            self.runtime_log_file.write(f'[{timestamp}] {msg}\n')
    ...
    def save_video(self, data_ids, scenario_names=None):
        return self.video_recorder.save(data_ids=data_ids, scenario_names=scenario_names)

    def save_video_with_metadata(self, batch_metadata):
        return self.video_recorder.save(batch_metadata=batch_metadata)

    def save_eval_batch_summary(self, summary):
        if self.batch_log_file is None:
            return
        with open(self.batch_log_file, 'a', encoding='utf-8') as batch_log:
            batch_log.write(json.dumps(summary, ensure_ascii=False) + '\n')
            batch_log.flush()
