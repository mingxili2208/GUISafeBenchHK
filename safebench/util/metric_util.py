...
def cal_avg_yaw_velocity(sequence):
    ...
    time_diff = sequence[-1]['current_game_time'] - sequence[0]['current_game_time']
    if time_diff == 0:
        return 0

    avg_yaw_velocity = total_yaw_change / time_diff

    return avg_yaw_velocity


def get_route_scores(record_dict, time_out=30):
    valid_sequences = {data_id: sequence for data_id, sequence in record_dict.items() if sequence}
    ...
    for data_id, sequence in valid_sequences.items():
        if sequence[-1]['collision'] == Status.FAILURE:
            num_collision += 1
        sum_out_of_road_length += cal_out_of_road_length(sequence)

    collision_rate = num_collision / len(valid_sequences)
    out_of_road_length = sum_out_of_road_length / len(valid_sequences)
    ...
    avg_distance_to_route = total_distance_to_route / len(valid_sequences)
    route_completion = total_route_completion / len(valid_sequences)
    avg_time_spent = total_time_spent / len(valid_sequences)
