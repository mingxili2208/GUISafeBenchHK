...
| 崩溃信号 | 根因 |
|----------|------|
| **SIGSEGV (原始)** | EBADF 双 close 到达 `carla::throw_exception` → `UE_LOG(Fatal)` |
| **SIGABRT** | `throw std::runtime_error` 在 UE4 的 `-fno-exceptions` 环境下触发 `__cxxabiv1::failed_throw → abort()` |
| **SIGSEGV (0x90)** | `pthread_exit` 杀死 ASIO worker 线程 → epoll `descriptor_state*` 野指针 → 下次 `start_accept()` 在 `deregister_descriptor` 解引用 0x90 偏移 |
| 卡死 | `pthread_exit` 后 io_service 部分挂起，epoll 状态腐烂 |
...
## 修复 1：Carla.cpp — EBADF 干净终止（第三轮修正）

**文件**：`Unreal/CarlaUE4/Plugins/Carla/Source/Carla/Carla.cpp`

```cpp
#ifndef CARLA_THROW_EXCEPTION_FIX_V3
// 如果 Carla 仍未包含此修复，需手动应用以下变更
#endif

// 不再需要 #include <pthread.h>
  ...
  void throw_exception(const std::exception &e) {
    ...
    if (msg && std::strstr(msg, "close: Bad file descriptor") != nullptr) {
      UE_LOG(LogCarla, Error,
        ...
        UTF8_TO_TCHAR(msg));
      // std::terminate() → SIGABRT：干净、刻意的进程终止。
      // 对比：pthread_exit() 只杀当前线程，留下 epoll reactor 的
      // descriptor_state 野指针，导致下个 start_accept() 在
      // deregister_descriptor 中 SIGSEGV（访问偏移 0x90）。
      std::terminate();
    }
...
> **为什么从 `pthread_exit` 改为 `std::terminate()`**：
>
> `pthread_exit(nullptr)` 只杀 ASIO worker 线程，但**不调用任何 C++ 析构器**。
> epoll reactor 内部的 `descriptor_state` 对象残留在内存中。
> 当新连接到达并触发 `start_accept()` → `deregister_descriptor(descriptor_state*&)` 时，
> 解引用一个已释放/无效的指针 → 访问 `0x0000000000000090` → **SIGSEGV**。
>
> 正确的防线是**修复 2**（`close_socket_once()`）从源头阻止双 close。
> 如果 EBADF 仍到达此处，说明原子 close 未生效，此时进程必须干净终止，
> 而不是留下腐烂的 epoll 状态造成更隐晦的崩溃。
...
| 治标 | `std::terminate()` 替代 `pthread_exit` | 若 EBADF 仍到达 → 干净 SIGABRT，不破坏 epoll 状态 |
